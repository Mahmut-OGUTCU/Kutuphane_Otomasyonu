...
def emaneAlinanKitaplar(uye_id):
    db.mycursor.execute(
        "SELECT kitap_ISBN, kitaplar.kitap_ismi AS uye,\
...
def teslimEt(kitapISBN):
    db.mycursor.execute("UPDATE kutuphane_otomasyonu.emanet SET emanet_durumu = 1 WHERE kitap_ISBN= %s",(kitapISBN,))
    db.mydb.commit()